...
router.post('/invite', function(req, res) {
  if (req.body.email && (!config.inviteToken || (!!config.inviteToken &&
      req.body.token === config.inviteToken))) {
    request.post({
      ...
      if (body.ok) {
        res.render('result', {
          community: config.community,
          message: 'Success! Check "' + req.body.email +
        ...
        if (error === 'already_invited' || error === 'already_in_team') {
          res.render('result', {
            ...
            message: 'Success! You were already invited.<br>' +
              'Visit to <a href="https://' + config.slackUrl + '">' +
              config.community + '</a>'
          });
          return;
